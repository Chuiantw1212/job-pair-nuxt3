...
async function openSeoDialog() {
    const response = await repoOrganizationDesign.postItem(state.organizationDesign)
}
async function initializeCompanyInfo() {
    // 生成本地端company資料
    const { user } = repoAuth.state.organizationDesign
    if (!user || !user.id || state.organizationDesign.companyInfo.id) {
        return
    }
    // Load from store
    let companyInfo = getDefaultCompany()
    if (repoAuth.state.organizationDesign.company) {
        companyInfo = JSON.parse(JSON.stringify(repoAuth.state.organizationDesign.company))
    } else {
        ...
        if (companyRes.data === false) {
            ...
            state.organizationDesign.isNewCompay = true
        }
        if (companyRes.data) {
    ...
    state.organizationDesign.companyInfo = companyInfo
    state.organizationDesign.companyLogo = companyInfo.logo
    state.organizationDesign.companyBanner = companyInfo.banner
    state.organizationDesign.companyImages = companyInfo.images ?? []

    const descriptionRef = currentInstance.refs.descriptionRef
    const jobBenefitsRef = currentInstance.refs.jobBenefitsRef
    if (descriptionRef) {
        descriptionRef.setData(state.organizationDesign.companyInfo.description)
    }
    if (jobBenefitsRef) {
        jobBenefitsRef.setData(state.organizationDesign.companyInfo.jobBenefits)
    }
    setWelfareFlags()
}
...
function setWelfareFlags() {
    if (!state.organizationDesign.companyInfo) {
        return
    }
    const { jobBenefits = "" } = state.organizationDesign.companyInfo
    let welfareCopy = JSON.parse(JSON.stringify(jobBenefits))
    state.organizationDesign.benefitFlags.forEach((welfareType) => {
        const labels = jobBenefitsConfig[welfareType]
        state.organizationDesign.jobBenefits[welfareType] = labels.filter((keyword) => {
            return welfareCopy.includes(keyword)
        })
        state.organizationDesign.jobBenefits[welfareType].forEach((label) => {
