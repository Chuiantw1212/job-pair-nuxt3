    ...
    <div class="profile">
        <template v-if="state.profile">
            ...
            <LazyMoleculeProfileCard id="profileBasic" name="基本求職資料" class="mt-3">
                ...
                <LazyAtomInputUploader v-model="state.profile.resumes" class="mt-3" name="履歷" :size="5242880"
                    :accept="'.pdf'" :max="3" :required="device.state.isLarge" :getFileBuffer="getUserResume">
                </LazyAtomInputUploader>
                <LazyAtomInputCkeditor name="個人簡歷" v-model="state.profile.description" hint="此區塊將會揭露給企業端參考" class="mt-3"
                    :required="state.profile.isActive" placeholder="請概述您過往的學經歷，凸顯個人優勢與專業領域，讓企業主對您留下深刻的第一印象。" :hasBtn="true"
                    ref="description">
