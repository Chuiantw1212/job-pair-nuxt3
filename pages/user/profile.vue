<template>
    <div v-if="state.profile" class="state.profile">
        ...
        <ProfileCard name="基本資料" class="mt-3">
            <div class="d-flex">
                <InputPhotoSingle v-model="state.profile.image" name="大頭貼" @update:modelValue="uploadImage($event)">
                </InputPhotoSingle>
            </div>
            <InputText name="姓名" v-model="state.profile.name" :disabled="user && !!user.name"></InputText>
            <InputEmail name="Email" v-model="state.profile.email" class="mt-3" disabled></InputEmail>
            <InputText name="手機號碼" v-model="state.profile.telephone" placeholder="請輸入手機" class="mt-3" required>
            </InputText>
            <InputCalendar name="生日" v-model="state.profile.birthDate" class="mt-3" :disabled="user && !!user.birthDate"
                required>
            </InputCalendar>
            <InputSelect name="性別" v-model="state.profile.gender" :items="selectByQueryRes.gender" class="mt-3"
                required>
            </InputSelect>
            <InputSelect name="最高學位" v-model="state.profile.educationLevel" :items="selectByQueryRes.educationLevel"
                class="mt-3">
            </InputSelect>
            <ProfileSelectContainer v-model="filterOpen.educationCategory" name="學科分類" :max="1" class="mt-3">
                <template v-slot:header>
                    <ProfileSelectLabels v-model="state.profile.educationCategory"
            ...
            <div class="profile__management">
                帳戶管理
                <DeleteModal class="managemement__others"></DeleteModal>
            </div>
            <BtnSimple class="mt-2" :style="{ width: '145px' }" @click="logout()">登出</BtnSimple>
        </ProfileCard>
        <ProfileCard name="求職資訊" class="profile__information profile__doc mt-3 ">
            <ProfileSelectContainer v-model="filterOpen.occupationalCategory" name="欲申請職務類別" :max="3" required>
                <template v-slot:header>
                    <ProfileSelectLabels v-model="state.profile.occupationalCategory" :jobCategoryMap="jobCategoryMap"
                        :items="jobCategory">
                    </ProfileSelectLabels>
                </template>
                <template v-slot:body>
            ...
            <InputCheckSingle class="information__isActive mt-3" v-model="state.profile.isActive" name="目前求職狀態">
                <span class="isActive__desc">若有適合的職缺，我願意讓企業主主動寄信給我</span>
            </InputCheckSingle>
            <InputTextarea name="個人簡歷" v-model="state.profile.description" hint="此區塊將會揭露給企業端參考"
                class="resume__introduction mt-3" rows="8" placeholder="請概述您過往的學經歷，凸顯個人優勢與專業領域，讓企業主對您留下深刻的第一印象。"
                :required="state.profile.isActive">
            </InputTextarea>
        </ProfileCard>
        <ProfileCard name="履歷作品集" class="profile__information profile__doc mt-3 ">
            <InputUploader v-model="state.profile.resumes" :size="5242880" :accept="'.pdf'" :max="3" required>
            </InputUploader>
            <InputPortfolioVue v-model="state.profile.portfolio"></InputPortfolioVue>
        </ProfileCard>
        <div class="profile__footer">
            <BtnSimple :style="{ width: '205px' }" class="mt-3" @click="handleSubmit()">儲存</BtnSimple>
        </div>
    </div>
</template>
<script setup>
